...
const LocationModal = ({ openModal, onCloseModal, selectedLocationid }) => {
    ...
    const fetchData = async () => {
        ...
        if (selectedLocationid) {
            try {
                const data = await apiService.fetchData(`api/location/${selectedLocationid}`, jwtToken);
    ...
    const handleSubmitReview = async (event) => {
        ...
        try {
            await apiService.postData1(`api/reviews/${locationData?.id}`, reviewData, jwtToken);
            await fetchData();
        } catch (error) {
        ...
        <div className={`overflow-y-auto overflow-x-hidden bg-gray-500 bg-opacity-75 fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-[calc(100%-1rem)] max-h-full max-h-full ${openModal ? 'block' : 'hidden'}`}>
            <div className='flex justify-center'>
                ...
                <div className="relative p-4 w-full max-w-2xl bg-white-500 p-6">
                    ...
                    <h1 className='text-[40px] font-bold'>{locationData?.name}</h1>
                    <div className='py-3'>
                        <img src={locationData?.image_link} className='w-full h-full' style={{ borderRadius: "5px" }} />
                    </div>

                    <a className='flex items-center py-1' href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(locationData?.name)}`} target="_blank" rel="noopener noreferrer">
                        ...
                        <div className='ml-3'>{locationData?.address}</div>
                    </a>
                    <div className='flex items-center py-1'>
                        ...
                        <div className='ml-3'>{locationData?.avg_rate}</div>
                    ...
                    <div className='flex py-1 items-center'>
                        ...
                        <div className='px-3'>
                            {locationData?.phone_number ? locationData?.phone_number : "Không có SĐT"}
                        </div>
                    </div>
                    ...
                    <div className="relative overflow-x-auto block md:hidden">
                        {Object.keys(dayOfWeek).map((day, index) => (
                            <div key={index} className="py-1 px-10 ">
                                ...
                                <span className="ml-2">{locationData?.business_hours[day] ? locationData?.business_hours[day] : "Không có"}</span>
                            </div>
                        ))}
                    </div>
                            ...
                            <tbody>
                                <tr class="bg-white dark:bg-gray-800 dark:border-gray-700">
                                    <th class="px-3 py-3 whitespace-nowrap">
                                        {locationData?.business_hours?.monday ? locationData?.business_hours?.monday : "Không có"}
                                    </th>
                                    <td class="px-3 whitespace-nowrap">
                                        {locationData?.business_hours?.tuesday ? locationData?.business_hours?.tuesday : "Không có"}
                                    </td>
                                    <td class="px-3 whitespace-nowrap">
                                        {locationData?.business_hours?.wednesday ? locationData?.business_hours?.wednesday : "Không có"}
                                    </td>
                                    <td class="px-3 whitespace-nowrap">
                                        {locationData?.business_hours?.thursday ? locationData?.business_hours?.thursday : "Không có"}
                                    </td>
                                    <td>
                                        {locationData?.business_hours?.friday ? locationData?.business_hours?.friday : "Không có"}
                                    </td>
                                    <td className='px-3 whitespace-nowrap'>
                                        {locationData?.business_hours?.saturday ? locationData?.business_hours?.saturday : "Không có"}
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap">
                                        {locationData?.business_hours?.sunday ? locationData?.business_hours?.sunday : "Không có"}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <section class="bg-white dark:bg-gray-900 py-8 lg:py-16 antialiased">
                            <div class="max-w-2xl mx-auto px-4">
                                <div class="flex items-center mb-6">
                                    ...
                                    <div>{locationData?.reviews?.length} Bình Luận</div>                            </div>
                                ...
                                <article class="p-6 text-base bg-white rounded-lg dark:bg-gray-900">
                                    <div>
                                        {locationData?.reviews.reverse().map((review, index) => (
